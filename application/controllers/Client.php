...
///////////Start customer //////////
    ...
    public function save_customer()
    {
        $customer_info = array(
            ...
            'customer_cr_amount' => $this->input->post('customer_crl'),
...
///////////Start product//////////
    ...
    public function add_product()
    {
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            if($this->input->post('barcode')){
                $product_barcode = $this->input->post('barcode');
            }else{
                 $last_product = $this->inventory_model->get_last_product_id();
                if ($last_product) {
                    $product_code = $last_product->id+1;
                }else{
                    $product_code = 1;
                }
                $product_barcode = str_pad($product_code, 10, '0', STR_PAD_LEFT);
            }
            ...
            $product = array(
                ...
                'product_discount_per' => $this->input->post('discount'),
    ...
    public function duplicate_product_id($product_id)
    {
        $data = array();
        $data['main'] = true;
        $data['manufacturer_info'] = $this->inventory_model->get_manufacturer();
        ...
        $update_product = $this->inventory_model->get_product_by_id($product_id);
        $data['update_product'] = $update_product;
        $data['main_content'] = $this->load->view('home/product_duplicate_form', $data,true);
        $this->load->view('home/client_home', $data);
    }
    ...
    public function search_product_detail()
    {
        $data = array();
        $data['main'] = true;
        $data['main_content'] = $this->load->view('home/search_product_detail', $data, true);
        $this->load->view('home/client_home', $data);
    }

    public function search_product_detail_data()
     {
     $query = $this->input->post('txt');
     //echo $query;
     if ($query !== "") {
        
      $datap = $this->inventory_model->get_product_code_data($query);
        if ($datap) {
              
        $data = array();
        $data['main'] = true;
        $data['product_info'] = $datap;
        $this->load->view('home/product_search_detail', $data);
         } 
     }
    ...
    public function save_purchase()
        {
            ...
            $this->inventory_model->update_product_id($product, $product_id[$i]);
                ...
                'supplier_payment_detail' => $purchase_details,
    ...
    public function sales_invoice_entry()
        {
            ...
            if($last_invoice){
                ...
                $last_payment_receipt = $this->inventory_model->get_customer_payment_no();
                if($last_payment_receipt->customer_payment_no){
                        $customer_payment_no = $last_payment_receipt->customer_payment_no+1;
                    }else{
                        $customer_payment_no = 100001;
                    }
            ...
            if($temp != 0){
                ...
                'customer_payment_no' => $customer_payment_no ,
                'client_id' => $client_id,
                'customer_id' => $customer_id,
                'customer_payment_date' => $sales_date,
                'customer_payment_amount' => $paid_amount,
                'customer_payment_detail' => $sales_details,
    ...
    public function supplier_invoice_payment_entry()
    {
        ...
        $payment_id = "SPI-".uniqid();
         ...
         $supplyer_payment = array(
                ...
                'supplier_payment_detail' => $payment_note,
                'supplier_payment_entry_by' => $this->session->userdata('user_id'),
                'supplier_payment_entry_date' => date('Y-m-d'),
                'supplier_payment_created_at' => date('Y-m-d H:i:s'),
                'supplier_payment_status' => 1
  ...
  //customer payment Entry save
    public function customer_invoice_payment_entry()
    {
        ...
        $balance_payment_amount = 0;
            ...
            $qty =count($sales_invoice_id);
            for($i=0; $i<$qty;$i++){
            $balance_payment_amount += $balance_payment[$i];
              $invoice_info = $this->inventory_model->get_sales_invoice($sales_invoice_id[$i]);
              $payment_total += $payment_amount[$i];
              $payment_info = array(
                'sales_payment_id' => "PPI-".uniqid(),
        ...
        $customer_payment = array(
                ...
                'customer_payment_detail' => $payment_note,
                'customer_payment_entry_by' => $this->session->userdata('user_id'),
                'customer_payment_entry_date' => date('Y-m-d'),
                'customer_payment_created_at' => date('Y-m-d H:i:s'),
                'customer_payment_status' => 1
    ...
    if ($balance_payment_amount > 0) {
        if ($check_balance) {
                $current_balance = $check_balance->customer_transaction_balance-$balance_payment_amount;
            
        }else{
           
                $current_balance = -$balance_payment_amount;
        }
        ...
        $transaction_info = array(
            ...
            'customer_id' => $customer_id,
            'customer_transaction_type' => "Credit(-)",
            'customer_transaction_amount' => $balance_payment_amount,
            'customer_transaction_balance' => $current_balance,
            'customer_transaction_description' => "Due Payment Adjustment",
            'customer_transaction_entry_by' => $this->session->userdata('user_id'),
            'customer_transaction_entry_date' => date('Y-m-d'),
            'customer_transaction_created_at' => date('Y-m-d H:i:s'),
            'customer_transaction_status' => 1
...
/////////Start Report//////////// 
   //all report toady
    ...
    public function top_selling_product_report()
    {
        $data = array();
        $data['main'] = true;
        $client_id = $this->session->userdata('client_code');
        ...
        $data['title'] = 'Top '.$qty.' best selling products from '.date("d/m/Y", strtotime($from_date)).' to '.date("d/m/Y", strtotime($to_date));
        $data['client_info'] = $this->inventory_model->get_client_by_id($client_id);
        $data['products'] = $this->inventory_model->get_top_selling_product($from_date, $to_date, $qty);
        $data['main_content'] = $this->load->view('home/report_top_selling_product', $data,true);
        $this->load->view('home/client_home', $data);
    }

    public function idle_product_report()
    {
        $data = array();
        $data['main'] = true;
        $client_id = $this->session->userdata('client_code');
        $data['title'] = 'Idle products';
        $data['client_info'] = $this->inventory_model->get_client_by_id($client_id);
        $data['products'] = $this->inventory_model->get_idle_product();
        $data['main_content'] = $this->load->view('home/report_idle_product', $data,true);
        $this->load->view('home/client_home', $data);
    }        
/////////End Report////////////
 ...
 public function product_ledger()
{
    ...
    public function sales_product_code_data()
      ...
      public function product_search()
     {
      ...
      $data = $this->inventory_model->fetch_search_product($query);
       ...
       foreach($data->result() as $product)
       {
        ...
        $output .= '
          <tr>
           ...
           <td>'.$product->product_stock.' '.$product->unit_name.'</td>
           <td>'.number_format((float)$product->product_unit_price, 2, '.', '').'</td>
           <td>'.number_format((float)$product->product_unit_mrp, 2, '.', '').'</td>
           <td>'.number_format((float)$product->product_stock*$product->product_unit_price, 2, '.', '').'</td>
           <td><img alt="..." onerror="this.src='.base_url().'assets/dist/img/qr.png" src="'. base_url().$product->product_image.'" height="50" width="45" /></td>';

           if ($product->product_status == 1) {
                $output .= '<td><a class="btn btn-success btn-xs" href="'. base_url() . "deactivate-product/" . $product->product_id.'">
                  Active
                </a></td>';
           }else{
            $output .= '<td><a class="btn btn-danger btn-xs" href="'.base_url() . "activate-product/" . $product->product_id.'">
                  Deactivated
                </a></td>';
           }
           ...
           $output .= ' 
           <td>
            <a class="btn btn-primary btn-xs" href="'. base_url() . "view-product/" . $product->product_id.'">
                  View
                </a>
                <a class="btn btn-info btn-xs" href="'. base_url() . "edit-product/" . $product->product_id.'">
                  Edit
                </a>';
                if ($product->product_code) {
           $output .= '    <a class="btn btn-warning btn-xs" href="'.base_url() . "product-barcode/" . $product->product_id.'" target="_blank">
              Barcode
            </a>';
        }
     $output .= '<a class="btn btn-info btn-xs" href="'.base_url() . "duplicate-product/" . $product->product_id.'">
                  Duplicate
                </a> 
           </td>
          </tr>
