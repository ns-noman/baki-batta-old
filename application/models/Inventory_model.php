...
//sales//
    ...
    public function sales_customer_search($postData)
      {
         ...
         if(isset($postData['search']) ){
           ...
           foreach($records as $row ){
            ...
            $due = $this->get_customer_due($row->customer_id);
            if($due){
   ...
   // Get Last sales invoice no
    public function get_sales_invoice_no()
    {
     ...
     public function get_customer_payment_no()
    {
        $this->db->select('*');
...
//get customer payment
  ...
  // Get customer payment
    ...
    public function get_top_selling_product($from_date, $to_date, $qty)
    {
        $this->db->select('*');
        $this->db->select_sum('sales_item_quantity');
        $this->db->order_by("sales_item_quantity", "desc");
        $this->db->group_by('sales_items.product_id');
        $this->db->where('sales_item_date >=', $from_date);
        $this->db->where('sales_item_date <=', $to_date);
        $this->db->where('sales_items.client_id', $this->session->userdata('client_code'));
    ...
    public function get_idle_product()
    {
        $query = $this->db->query('SELECT product_id FROM sales_items');
        $array=array();
        ...
        $this->db->select('*');
        $this->db->from('products');
        $this->db->where_not_in('products.product_id', $array);
        $this->db->where('products.client_id', $this->session->userdata('client_code'));
        $this->db->join('setup_category', 'products.category_id = setup_category.product_category_id', 'left');
        $this->db->join('inventory_stock', 'products.product_id = inventory_stock.product_id');
 ...
 public function fetch_search_product($query)
     {
      $this->db->select("*");
      $this->db->from("products");
      ...
      {
       $this->db->group_start();
        $this->db->or_like('product_name', $query, 'both');
        $this->db->or_like('product_code', $query, 'both');
        $this->db->or_like('product_model', $query, 'both');
        $this->db->or_like('product_sn', $query, 'both');
        $this->db->group_end();
